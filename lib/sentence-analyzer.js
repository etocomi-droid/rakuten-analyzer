...
/**
 * 類似文をグルーピングして件数集約
 * 簡易的にアスペクト+主要キーワードの一致でグループ化
 */
function groupSimilarSentences(items) {
    const groups = [];

    for (const item of items) {
        const normalized = item.sentence.replace(/[。！？!?\s]/g, '');
        let foundGroup = false;

        for (const group of groups) {
            // 短い方の文字列が長い方に含まれるか、または70%以上の文字が一致
            ...
            if (longer.includes(shorter) || calcSimilarity(normalized, gNorm) > 0.6) {
                group.count++;
                foundGroup = true;
                break;
            }
    ...
    // normalizedKey を削除してソート
    return groups
        .map(({ normalizedKey, ...rest }) => rest)
        .sort((a, b) => b.count - a.count);
}

/**
 * 簡易的な文字列類似度計算（bigram一致率）
 */
