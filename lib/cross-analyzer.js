...
function estimateCategoryAndPriceRange(products) {
    ...
    // カテゴリー判定: まずスクレイピングで取得したcategories配列を使う
    let category = '';
    const allCategoryArrays = products.map(p => p.categories || []);
    const nonEmpty = allCategoryArrays.filter(arr => arr.length > 0);
    if (nonEmpty.length > 0) {
        // 最初の商品のカテゴリー階層を「> 」区切りで表示、最大3階層
        const firstCats = nonEmpty[0].slice(0, 3);
        category = firstCats.join(' > ');
    }

    // カテゴリーが取れなかった場合、商品名の共通キーワードで推定
    if (!category) {
...
function normalizeForGrouping(sentence) {
    return sentence
        .replace(/[。！？!?\s、,・「」『』（）()]/g, '')
        .replace(/です|ます|ました|でした|だった|ている|ていた|しています|されています/g, '')
        .replace(/^(また|そして|しかし|ただ|でも)/g, '')
        .substring(0, 30);
}
